...
export const predict = async (doc_id) => {
  let finalresponse="Success";
  for(let i=0;i<doc_id.length;i++){
    let response = await axios.post(baseUrl2+"?data=["+doc_id[i]+"]");
    let aging_bucket="nan";
    if(response.data.length>0){
    aging_bucket = response.data[0].aging_bucket;}
    let reaponse2 = await axios.get(baseUrl + "PUpdateInvoiceData", {params: {doc_id:doc_id[i],aging_bucket:aging_bucket}});
    if(reaponse2.data.Status==="Error"){
      finalresponse=reaponse2.data.Status;
    }
  }
